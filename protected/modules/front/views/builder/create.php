...
<h1 class="heading">Create Builder Profile</h1>
...
<ul>
	<li><span><label for="Property_description" class="required">Property Type <span class="required">*</span></label></span>
		<div class="multi_checkbox avg">
			<?php 
				$projecttypes = isset($_POST['project_type_id'])? $_POST['project_type_id'] : null;
				echo CHtml::checkBoxList('project_type_id',$projecttypes,CHtml::listData(ProjectTypes::model()->findAll(),'id','project_type')); 
			?>
		</div>
	</li>
	<li class="error_message"><?php echo $form->error($projectType,'project_type_id'); ?></li>
</ul>
...
<ul>
	<li><span>Country<span class="required">*</span></span> <?php echo $form->dropdownList($model,'country_id',CHtml::listData(GeoCountry::model()->findAll(),'id','country'),array('empty'=>'All','class'=>'slctbox',
	                        'ajax' => array(
	                                'type'=>'POST',
	                                'url'=>CController::createUrl('/location/state/getList/page/builder'),  
...
<ul>
	<li><span>State<span class="required">*</span></span> <?php echo $form->dropdownList($model,'state_id',CHtml::listData(GeoState::model()->findAll('country_id=:country_id',array(':country_id'=>$model->country_id)),'id','state'),array('empty'=>'All','class'=>'slctbox',
		                        'ajax' => array(
		                                'type'=>'POST',
		                                'url'=>CController::createUrl('/location/city/getList/page/builder'),  
		                                'update'=>'#city_content',
										'data'=>'js:jQuery(this).serialize()',
	))
	);
...
<div id="city_content">
<ul>
	<li><span>City<span class="required">*</span></span> <?php echo $form->dropdownList($model,'city_id',CHtml::listData(GeoCity::model()->findAll('state_id=:state_id',array(':state_id'=>$model->state_id)),'id','city'),array('empty'=>'All','class'=>'slctbox'));
